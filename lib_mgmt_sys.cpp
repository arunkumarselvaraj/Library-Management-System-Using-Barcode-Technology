...
void book_deposit()
{
    ...
    while(fp.read((char*)&st,sizeof(student)) && found==0)
    {
	    if(strcmpi(st.retadmno(),sn)==0)
	    {
		    ...
		    if(st.rettoken()==1)
		    {
				while(fp1.read((char*)&bk,sizeof(book))&& flag==0)
				{
				  	if(strcmpi(bk.retbno(),st.retstbno())==0)
					{
						bk.show_book();
						flag=1;
						cout<<"\n\nenter the number of days till date from the issual of the book.. ";
						cin>>day;
						if(day>15)
						{
						   fine=(day-15)*1;
						   cout<<"\n\nFine amount is Rs. "<<fine;
						}
						st.resettoken();
						int pos=-1*sizeof(st);
						fp.seekp(pos,ios::cur);
			...
			cout<<"No book is issued to this student.. please check!!";
		}
	}
    if(found==0)
		cout<<"\nno Student with the given admission number..";
	getch();
	fp.close();
	fp1.close();
}
